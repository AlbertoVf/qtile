...
def separator(fg=theme["color1"], bg=theme["background"], size=50):
    return widget.Sep(padding=8, margin=8, foreground=fg, background=bg, size_percent=size, )


def systray(size=18):
    return widget.Systray(icon_size=size, margin=8, padding=8, )


def current_layout():
    return widget.CurrentLayout(font=f"{font} Bold", foreground=theme["color1"], )


def window_name():
    return widget.WindowName(font=f"{font} Bold Italic", format="{name}", max_chars=90, foreground=theme["active"], )
